...
namespace {
    ...
    void set_pose6d(ImuProcess::Pose6D& p, const double t, const Eigen::Vector3d& a,
        const Eigen::Vector3d& g, const Eigen::Vector3d& v, const Eigen::Vector3d& pos,
...
bool ImuProcess::process(MeasureGroup& meas, StatesGroup& state, PointCloudT::Ptr& cloud_out) {
    if (!imu_en) {
...
void ImuProcess::undistort_pcl(
    const MeasureGroup& meas, StatesGroup& state, PointCloudT::Ptr& out) {
    ...
    for (size_t i = 0; i + 1 < v_imu.size(); ++i) {
        ...
        imu_pose_.push_back({ });
        set_pose6d(imu_pose_.back(), offs_t, acc_imu, angvel_avr, vel_imu, pos_imu, R_imu);
    ...
    for (auto it_kp = imu_pose_.end() - 1; it_kp != imu_pose_.begin(); --it_kp) {
        ...
        for (; it_pcl->curvature / 1000.0 > head->offset_time; --it_pcl) {
            ...
            const Eigen::Vector3d P_compensate =
                (extR_Ri * (R_i * (r_lidar_imu_ * P_i + t_lidar_imu_) + T_ei) - exrR_extT);
...
void ImuProcess::forward_without_imu(
    ...
    if (pcl_wait_proc_.points.empty()) {
        return;
    }

    const double pcl_end_offset_time = pcl_wait_proc_.points.back().curvature / 1000.0;
