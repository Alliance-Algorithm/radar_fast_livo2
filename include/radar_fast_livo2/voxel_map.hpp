...
namespace std {
template <> struct hash<radar::fast_livo2::VOXEL_LOCATION> {
...
// ── 前向声明 ─────────────────────────────────────────────────────
void calcBodyCov(
    const Eigen::Vector3d& pb, float range_inc, float degree_inc, Eigen::Matrix3d& cov);
...
public:
    ...
    VoxelOctoTree(int max_layer, int layer, int points_size_threshold, int max_points_num,
...
public:
    ...
    void TransformLidar(const M3D& rot, const V3D& t, const PointCloudT::Ptr& input_cloud,
    ...
    /// 清理超出边界的体素
    void clearMemOutOfMap(const int& x_max, const int& x_min, const int& y_max, const int& y_min,
