...
struct Feature {
    ...
    Feature(Eigen::Vector2d px, Eigen::Vector3d f, cv::Mat img, M3D rot, V3D pos, int level)
        ...
        , level_(level)
        , img_(std::move(img))
        , T_f_w_rot_(std::move(rot))
        , T_f_w_pos_(std::move(pos)) { }
};
...
public:
    ...
    explicit VisualPoint(const V3D& pos)
        : pos_(pos) { }
...
struct SubSparseMap {
    ...
    std::vector<std::vector<float>>
        warp_patch;                         // 每点的 warp 后参考 patch（含全部金字塔层，扁平存储）
...
public:
    ...
    void processFrame(const cv::Mat& img, std::vector<pointWithVar>& pg,
...
private:
    ...
    [[nodiscard]] bool isInFrame(const Eigen::Vector2d& px, int boundary) const {
        return px.x() >= boundary && px.x() < width_ - boundary && px.y() >= boundary
            && px.y() < height_ - boundary;
    }

    // ── 核心算法函数（对应 FAST-LIVO2 src/vio.cpp） ──────────────────
    // 返回 false 表示 p.z() 过小（点在相机后方或退化），J 已清零，
    // 调用方（updateState）应跳过该点本次迭代的贡献。
    [[nodiscard]] bool computeProjectionJacobian(
        const V3D& p, Eigen::Matrix<double, 2, 3>& J) const;

    void getImagePatch(
        const cv::Mat& img, const Eigen::Vector2d& pc, float* patch_tmp, int level) const;

    static void getWarpMatrixAffine(const VIOManager& self, const Eigen::Vector2d& px_ref,
    ...
    void getWarpMatrixAffineHomography(const Eigen::Vector2d& px_ref,
    ...
    [[nodiscard]] static double calculateNCC(
        const float* ref_patch, const float* cur_patch, int patch_size);
    ...
    [[nodiscard]] cv::Mat cropAroundPixel(
        const cv::Mat& img, const Eigen::Vector2d& pc, Eigen::Vector2i& origin_out) const;

    // ── 每帧流水线阶段 ────────────────────────────────────────────
    void retrieveFromVisualSparseMap(const cv::Mat& img, const std::vector<pointWithVar>& pg,
        const std::unordered_map<VOXEL_LOCATION, std::unique_ptr<VoxelOctoTree>>& plane_map);
    ...
    void generateVisualMapPoints(const cv::Mat& img, std::vector<pointWithVar>& pg);
